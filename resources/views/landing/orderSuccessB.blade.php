            ...
            <table class="w-full text-sm text-left text-gray-500 mt-10">
                ...
                <tbody>
                    ...
                    @php
                        ...
                        $items[] = [
                            "id" => $order_product->product_id,
                            "name" => $order_product->Product->title ?? 'N/A',
                            "brand" => env('APP_NAME'),
                            "category" => isset($order_product->Product->categories[0]) ? $order_product->Product->categories[0]->name : env('APP_NAME'),
                            "list_position" => ($key + 1),
                    ...
                    @endphp
                        <tr>
                            ...
                            <td class="tbl-line-height">
                                {{ $order_product->Product->title ?? 'n/a' }}
                                @if($order_product->ProductData && $order_product->ProductData->attribute_items_string)
                                <br>
                                <p class="text-red-400 text-xs">{{$order_product->ProductData->attribute_items_string}}</p>
                                @endif
                            </td>
                            <td class="tbl-line-height">৳{{ $order_product->sale_price }}</td>
                            <td class="tbl-line-height">{{$order_product->quantity}}</td>
                            <td class="tbl-line-height text-right">৳{{ $order_product->sale_price * $order_product->quantity }}</td>
                        </tr>
                    @endforeach
                    ...
                    <tr>
                        ...
                        <td class="tbl-line-height text-right">৳{{ $order->shipping_charge }}</td>
                    </tr>

                    <tr class="mb-5">
                        <td colspan="2" class="tbl-line-height pt-4"></td>
                        <td colspan="2" class="tbl-line-height pt-4">Grand total</td>
                        <td class="tbl-line-height text-right pt-4">৳{{ $order->grand_total }}</td>
                    </tr>
