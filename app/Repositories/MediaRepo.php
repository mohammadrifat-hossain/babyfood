...
class MediaRepo {
    public static function upload($file){
        ...
        if(strpos($file->getMimeType(), 'image') !== false){
            ...
            if(env('APP_IMAGE_WEBP')){
                $image_resize = Image::make($file->getRealPath());
                $image_resize->fit($small_width, $small_height);
                $image_resize->encode('webp', 70);
                $image_resize->save(public_path($path . 'small_' . $filename . '.webp'));
            }
            ...
            if(env('APP_IMAGE_WEBP')){
                ...
                $image_resize->fit($medium_width, $medium_height);
                $image_resize->encode('webp', 70);
                $image_resize->save(public_path($path . 'medium_' . $filename . '.webp'));
            }
            ...
            if(env('APP_IMAGE_WEBP')){
                $image_resize = Image::make($file->getRealPath());
                $image_resize->fit($large_width, $large_width);
                $image_resize->encode('webp', 70);
                $image_resize->save(public_path($path . 'large_' . $filename . '.webp'));
            }
        }
        ...
        if(env('APP_IMAGE_WEBP')){
            $image_resize = Image::make($file->getRealPath());
            $image_resize->encode('webp', 70);
            $image_resize->save(public_path($path . $filename . '.webp'));
        }
    ...
    public static function sizes($image_path, $file_name){
        ...
        if(file_exists(public_path("uploads/$image_path/$file_name" . $prefix_extension))){
            $output['original'] = asset("uploads/$image_path/$file_name" . $prefix_extension);
        }else{
            $output['original'] = asset('img/no-image.png');
        }

        // dd(public_path("uploads/$image_path/small_$file_name"));
        if(file_exists(public_path("uploads/$image_path/small_$file_name" . $prefix_extension))){
            $output['small'] = asset("uploads/$image_path/small_$file_name" . $prefix_extension);
        }else{
            $output['small'] = asset('img/no-image.png');
        }
        if(file_exists(public_path("uploads/$image_path/medium_$file_name" . $prefix_extension))){
            $output['medium'] = asset("uploads/$image_path/medium_$file_name" . $prefix_extension);
        }else{
            $output['medium'] = asset('img/no-image.png');
        }
        if(file_exists(public_path("uploads/$image_path/large_$file_name" . $prefix_extension))){
            $output['large'] = asset("uploads/$image_path/large_$file_name" . $prefix_extension);
        }else{
