...
// Moneris Payment Gateway(https://developer.moneris.com)
// https://github.com/Moneris/eCommerce-Unified-API-PHP/blob/master/mpgClasses.php
class mpgClasses{
    public static function pay($order, $car_number, $cvc, $expire_month, $expire_year, $ref){
        /************************ Request Variables ***************************/
        // $store_id='gwca041782';
        // $api_token='izFclCoizPNtYid2l28N';
        $store_id = env('MONERIS_STORE_ID', 'store5');
        $api_token = env('MONERIS_API_TOKEN', 'yesguy');

        /********************* Transactional Variables ************************/
        $type = 'purchase';
        $order_id = 'ord-' . $order->id . date("dmy-G:i:s");
        $cust_id = $order->user_id;
        $amount = $order->grand_total;
        $pan = $car_number;
        $expiry_date = $expire_month . $expire_year; // MMYY
        $crypt = '7';
        $cvd = $cvc;
        ...
        foreach($order->OrderProducts as $key => $order_product){
            $item = array(
                ...
                'product_code' => $order_product->product_id,
                'extended_amount' => $order_product->sale_price
            );
        ...
        $mpgRequest->setTestMode(env('MONERIS_TEST_MODE', true)); //false or comment out this line for production transactions
        /************************ HTTPS Post Object ***************************/
        ...
        if(auth()->check()){
            if($ref == 'admin'){
                $route = route('back.orders.show', $order->id);
            }else{
                $route = route('user.order', $order->id);
            }
        }else{
        ...
        if($mpgResponse->getResponseCode() == "027"){
            ...
            // Order Payment
            OrderRepo::payment($order->id, $order->grand_total, $order_id, $mpgResponse->getTxnNumber(), 'Order Payment');
            ...
            foreach($order->OrderProducts as $order_product){
                $stock_note = 'Customer order payment.';
            ...
            $app_title = Info::Settings('general', 'title') ?? env('APP_NAME');
            Mail::to($order->email, $order->full_name)->bcc(env('ADMIN_NOTIFY_MAIL'))->send(new OrderConfirm("New Order #$order->id - $app_title", $order, false));
            // Sales Notification
            Mail::to(env('SALES_NOTIFY_MAIL'))->send(new OrderConfirm("New Order #$order->id - $app_title", $order, true));

            return redirect($route)->with('success-alert2', 'Order payment successful.');
        }

        if($mpgResponse->getMessage() == 'DECLINED           * CARD PROBLEM       =CARD PROBLEM'){
            return redirect($route)->with('error-transaction', 'Invalid card information!');
        }
        return redirect($route)->with('error-transaction', $mpgResponse->getMessage());
    ...
    public static function refund($order_payment, $amount){
        $store_id = env('MONERIS_STORE_ID', 'store5');
        $api_token = env('MONERIS_API_TOKEN', 'yesguy');
        $orderid = $order_payment->gateway_order_id;
        $txnnumber = $order_payment->txn_number;
        $dynamic_descriptor = $order_payment->note;
        ...
        if($mpgResponse->getResponseCode() == "027"){
            ...
            // // Send mail to admin and customer
            // $app_title = Info::Settings('general', 'title') ?? env('APP_NAME');
            // Mail::to($order->email, $order->full_name)->bcc(env('ADMIN_NOTIFY_MAIL'))->send(new OrderRefundMail("Order Refunded #$order->id - $app_title", $order, $order_payment->amount));

            return [
                'success' => true,
                'tnx_number' => $mpgResponse->getTxnNumber(),
                'message' => $mpgResponse->getMessage(),
